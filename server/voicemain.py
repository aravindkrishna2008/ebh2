...
def process_mp3(filename, output_filename=None):
    ...
    # Generate Rap, 1 random variation
    genre = 'rap'
    variation = random.randint(0, 4)
    print(f"Selected variation: {variation}")
    
    processor.generate_music(genre=genre, variation=variation)
    
    if output_filename:
        processor.mix(os.path.basename(output_filename))
        generated_file = os.path.join(processor.output_dir, os.path.basename(output_filename))
        if os.path.abspath(generated_file) != os.path.abspath(output_filename):
            import shutil
            shutil.move(generated_file, output_filename)
    else:
        processor.mix(f"{genre}_{variation}.wav")
